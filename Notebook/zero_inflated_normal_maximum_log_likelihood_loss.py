...
## Defining function: Softplus Function
###########################################################################################################################
def softplus(x):
    """
    Compute the softplus (beta) of x
...
def forward_propagation(X, parameters):
  ...
  """
    ...
    a3_2 = z3[1,:]
    a3_3 = softplus(z3[2,:])
    a3 = np.concatenate([a3_1.reshape(1,-1), a3_2.reshape(1,-1), a3_3.reshape(1,-1)], axis=0)
...
def loss_function(a3, Y):
    ...
    def binary_cross_entropy_loss(y_true, y_pred):
        ...
        # Fetching parameters
        pb = y_pred[0,:].reshape(1,-1).T  # Probability vector

        # Calculating Binary Cross-Entropy Loss
        logprobs = np.multiply(-np.log(pb+epsilon), I1) + np.multiply(-np.log(1-pb+epsilon), 1 - I1)
        bce_loss =  np.sum(logprobs)

        return bce_loss

    def normal_maximum_loglikelihood_estimator_loss(y_true, y_pred):
        ...
        # Fetching parameters
        mu = y_pred[1,:].reshape(1,-1).T  # Mean vector
        sg = y_pred[2,:].reshape(1,-1).T  # Sigma vector

        # Defining constants
        k = np.log(2*np.pi)

        # Calculating Normal Maximum Log-Likelihood Loss
        mle_error = np.multiply((k + 2*np.log(sg) + ((y_true - mu)**2)/(sg**2))/2, I1)
        mle_loss = np.sum(mle_error)
    ...
    # Calculating Binary Cross-Entropy Loss
    bce_loss =  binary_cross_entropy_loss(y_true=Y, y_pred=a3)

    # Calculating Root Mean Squared Error
    mle_loss = normal_maximum_loglikelihood_estimator_loss(y_true=Y, y_pred=a3)

    # Calculating Total Cost
    total_loss = bce_loss + mle_loss
...
def backward_propagation(X, Y, cache):
    ...
    # This derivative considers that the first neuron in the last layer uses a sigmoid activation function and a BCE cost function
    dz3_0 =  (1/m) * ((1 - I1) / (1 - a3[0,:]) - I1 / a3[0,:]) * del_sigmoid(z3[0,:]) 
    # This derivative considers that the third neuron in the last layer uses a identity activation function and a Normal MLE cost function
    dz3_1 =  (1/m) * ((a3[1,:] - Y) / a3[2,:]**2) * I1
    # This derivative considers that the third neuron in the last layer uses a softplus activation function and a Normal MLE cost function
    dz3_2 =  (1/m) * (1 / a3[2,:] - ((a3[1,:] - Y)**2) / (a3[2,:]**3)) * I1 * del_softplus(z3[2,:])

    dz3 = np.concatenate([dz3_0.reshape(1,-1), dz3_1.reshape(1,-1), dz3_2.reshape(1,-1)], axis=0)
    dW3 = np.dot(dz3, a2.T)
